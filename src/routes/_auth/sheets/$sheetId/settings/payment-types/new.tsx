...
export const Route = createFileRoute('/_auth/sheets/$sheetId/settings/payment-types/new')({
...
function NewPaymentTypePage() {
  ...
  function handleSubmit(values: FormValues) {
    createMutation.mutate(
      { sheetId, createdBy: session!.user.id, name: values.name.trim(), icon: values.icon },
      {
        onSuccess: () =>
      ...
      <Box p="md">
        <form onSubmit={form.onSubmit(handleSubmit)}>
          <Stack gap="sm">
            ...
            />
            <Button type="submit" color="teal" loading={createMutation.isPending} mt="xs">
              Add payment type
            </Button>
          </Stack>
        </form>
